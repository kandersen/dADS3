...
\newcommand{\OkasakiLazy}{\textsc{Okasaki-Lazy}}
\newcommand{\OkasakiStrict}{\textsc{Okasaki-Strict}}
...
but all other operations require traversing the entire list. Below are
...
of the tail list as the potential function. Every insert into tail
...
be used, have been computed. The shared cost of an operation is the
...
step and also has to pay to the potential function, which is a total
...
\chapter{$\HoodMelville$ - $\BigO{1}$ worst case}
...
There are a few key insights to this algorithm. First, reversing a list
...
in establishing a useful invariant by proving that \lstinline{inject}
preserves the empty invariant, and the empty queue establishes it
...
We have a total of four conceptually different queues - where two comes in the two flavours, lazy or eager evaluated, for a total of six. We are
asked to design benchmarks where a queue is only used once and also
where the queue can be used multiple times. In this chapter we describe
the different tests that we apply to the queues and we discuss the
results of said tests. Per default, Haskell will evalute to first seen constructor, however, when doing tests this might give us some strange timing results. Therefore, we make sure that the queue under test is evaluated to normal form.

\section{One punch tests}

We have designed two tests that enable us to directly measure time of
insertions and insertion + deletion. However, a queue is useful for more things
than theoretic time analysis so we are also using the queue to do Breadth-First search in graphs to have a sort of real-life example.
...
\subsection{Time of insert + delete}

Rotation will not occur on the same points in execution based on test size, thus if we were to test time of delete by just deleting a few items when the queue has $x$ elements, we might force a rotation for one queue but not another. To counter this obstacle we decided to measure the total time of inserting $n$ elements and removing all elements again.
...
element is enqueued and popped exactly once for a total of $2^d$ operations for a tree of depth $d$.
...
persistent nature of queues, which is clearly explained in section 4.1.
...
delete and insert $c$ times on the queue given as argument, insert an element such that the size is increment by one ($n+1$) and the iteration is repeated. We stop when we have performed the $2 \cdot n$ iterations.

\subsection{Queue-sort}

We define a version of Queue-Sort that is not very efficient. Given $n$ queues
of size $n$ where elements are ordered by
...
In this section we present the results of our experiments. All running times has been measured and compared to a meanThe running time for the $\List$ grows much faster than the others, making it harder for us to distinguish between the rest of the queues. Therefore, the $\List$ running times are from all of our graphs, but the original graphs can be found in the appendix.

\subsection{Time of insert}

This test was supposed to measure the time a single insertion takes depending on the fill-rate and the time usage is plotted below: 

\input{_results_Inserts_nolist}

The results are completely as expected. There is only a slight overhead when cons'ing elements on a list and all takes a constant time which matches the theoretic bounds. 

\subsection{Time of insert + delete}

This test measures how quickly a queue can be filled and emptied again:

\input{_results_InsertsDeletes_nolist}

Again we can conclude that the theoretic bounds are met for deletion. Additionally, we see, that the Hood-Melville takes a bit longer per operation than the others. It was expected that maintaining the rotation would have a constant factor of penalty.

\subsection{Results of Breadth-First Search}

Below is the graph-plot of the Breadth-First Search results:

\input{_results_BST_nolist}

Interestingly, for larger levels, there is not much difference in the time per operation. This was not completely expected but since every node must be popped from the queue, every queue forces a complete evaluation.

For the Pairs queues there is exactly one rotation per level, which is easy to convince onself about, because when a level is completed, the tail list will contain all childs for the next level and the head list will be empty. 

For the Okasaki queues there will be two rotations for each level. We will give a simple argument. In the very best case, beginning on a new level, all elements $n$ will be in the head list $H$. This is the farthest from a rotation we can be. Since every dequeue will add two new elements to the tail a rotation will occur in the $n/2$ step when adding the left child to the queue. After rotation and the completed $n/2$ steps we have $|H| = n - n/2 + n - 1 = 3/2n-1$ and $|T| = 1$, with the remaining $n/2$ nodes of the level yet to be dequeued. After $n/2$ dequeues $|H|=n-1$ if no rotation happens but the tail would be $|T|=n+1$ and another rotation would therefore have happened. There is no way of avoiding at least two rotations per level.

But the penalty of rotating $n/2$ elements two times instead of $n$ elements one time is not significant, therefore we see similar results.

\subsection{Insert onto the same queue}

This is the first test that tries to force multiple expensive operations for each queue and the time-usage is shown below:

\input{_results_WorstCase_nolist}

We can see that the $\PairsStrict$ and $\OkasakiStrict$ performs a bit slower than their lazy counter-parts. This was expected but the difference between them is less than wat we anticipated. 
What is surprising is that $\HoodMelville$ is slower than all of them (not regarding $\List$ which is THE slowest), but thus must have something to do with the constant of insertions and deletions that is for this example is 10.

\subsection{Queue-Sort}

This is the results of our real-world use-case implemented as a real-world (not computer-scientist) programmer would do: 

\input{_results_QueueSort_nolist}

When the number of queues and the elements in the queues increases, the running time of each of the queues goes towards the same point. This is because all elements must be removed from each queue, and there will for each queue sans $\HoodMelville$ be a linear amount of expensive operations.
But towards $n=100$ we find that $\HoodMelville$ and $\PairsLazy$ seems to be the fastest.

\section{Summary on running times}

In general all the running times are pretty close when we exclude the $\List$ implementation. This matches the theoretic bounds but we must admit we had hoped for bigger gaps.

One of the reason why we did not achieve might be that everything was compiled with an optimizing compiler, which closer matches what would be used in the industry (if the used functional programming). It would not remove function calls, but it would inline them making them faster.
...
tree would contain the element, otherwise subtract the size from the
...
This is the last report in the course Advanved Data Structures at Aarhus University and for our last project we decided to work with functional data structures. We have shown four different implementations of queues.

When analyzing the queues time-complexities, we found that analyzing lazy evaluated structures required us to reformulate the potential function for amortized analysis.

We proved our implementations correct in Coq, which gave us the confidence to go ahead and actually perform experiments on them. Our experiments more or less showed that our theoretic bounds matched.

Finally, we wanted to show that it is possible to do $\BigO{\log n}$ indexing with constant time push and pop, which was an exercise posted in the theoretical project.
