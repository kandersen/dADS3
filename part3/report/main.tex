...
why we can say that our implentations are correct.

We then describe how we measure running times in a lazely evaluated
...
Because a queue pops elements from the front and injects elements in the
end, this is not the most efficient queue implementation.
...
Lazyness is a feature in Haskell. A queue might perform differently
...
enforce a strict evaluation of the datastructure. That is, a value
...
is lazy, giving us the desired evalution strategy.
...
evalutations with memoization, which we call call-by-need, the
...
assumption that all lazy constructions of the datastructure, that will
...
cases. The case where $\Pop$ does not force an un-evaluated expression
...
un-evaluated expression, the operation will take $\BigO{n}$, but the
potential will decrease by the same amount, giving us a total cost
of 2. Finally, if $\Pop$ rotates and forces an un-evaluted expression
...
new tail list. Dequeueing is bit harder because we are currently
...
The recopying (or rotation) should be completed before the first
...
on queues, which according to Hughes \& Claeseen \cite{CLA02} defines a
