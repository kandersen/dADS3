...
\section{Special cases for graphs }

Overall, the previous section provides a generel desription of when $\Dijkstra$ will perform a lot of operations, but one could be interested in graphs that has a many or few $\Insert$- or $\DecreaseKey$ operations.
...
For suitable $k_i$s and $c_j$ we can make $\Dijkstra$ call $\DecreaseKey$ for every node $n+1$ to $m$, for each of the nodes in the vertical list $2$ to $n$. If $n=m=|V|$ we will $\BigOh{n^2}$ calls to $\DecreaseKey$.
